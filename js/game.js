...
var viewportMode = {enable:false, viewport:'portrait', text:'Rotate your device <br/>to portrait'}; //device viewport mode, portrait or landscape
...
var coundownTimer = 60000; //countdown timer
...
//drag property
var dragRevertSpeed = .5; //revert speed
...
var scoreDisplayText = 'You scored : [NUMBER]!'; //score result display text
var timerDisplayText = 'Best time : [NUMBER]!'; //timer result display text

//Social share, [SCORE] will replace with game score
var shareEnable = true; //toggle share
var shareTitle = 'Highscore on Quiz BYOD4US02-dnd-Quiz-Game Game is [SCORE]pts';//social share score title
var shareMessage = '[SCORE] is my new high score on the BYOD4US02-dnd-Quiz-Game! Try it now! -'; //social share score message
	...
	switch(page){
		...
		case 'result':
			...
			if(scoreMode == 'score'){
				...
				$('#resultScore').html(timerDisplayText.replace('[NUMBER]', millisecondsToTime(playerData.timer)));
...
function loadQuestion(){
	...
	if(gameData.targetArray[gameData.sequenceNum].drag == 'true' && dragRandomAnswer && !$.editor.enable){
		randomAnswerLayout = true;	
	}
	...
	for(var t=0; t<2; t++){
		...
		var submitButton = -1;
		for(var n=0;n<loopTargetArray[gameData.sequenceNum].answer.length;n++){
			if(loopTargetArray[gameData.sequenceNum].answer[n].submit == 'false' || loopTargetArray[gameData.sequenceNum].answer[n].submit == undefined){
		...
		if(loopTargetArray[gameData.sequenceNum].background != ''){
			fileFest.push({src:loopTargetArray[gameData.sequenceNum].background, id:thisMode+'backgroundImage', type: createjs.LoadQueue.IMAGE})
		}
		
		if(loopTargetArray[gameData.sequenceNum].type == 'image'){
			fileFest.push({src:loopTargetArray[gameData.sequenceNum].question, id:thisMode+'questionImage', type: createjs.LoadQueue.IMAGE})
		}
		...
		for(var n=0;n<loopTargetArray[gameData.sequenceNum].answer.length;n++){
			if(loopTargetArray[gameData.sequenceNum].answer[n].type == 'image'){
				fileFest.push({src:loopTargetArray[gameData.sequenceNum].answer[n].text, id:thisMode+'answerImage'+n, type: createjs.LoadQueue.IMAGE})
			}
			
			if(loopTargetArray[gameData.sequenceNum].answer[n].dropLabelType == 'image'){
				fileFest.push({src:loopTargetArray[gameData.sequenceNum].answer[n].dropLabelText, id:thisMode+'answerLabelImage'+n, type: createjs.LoadQueue.IMAGE})
			}
			
			var answerNum = loopTargetSeqArray[n];
			var answerAudio = loopTargetArray[gameData.sequenceNum].answer[answerNum].audio;
			answerAudio = answerAudio == undefined ? '' : answerAudio;
			
			if(answerAudio != '' && checkBoolean(loopTargetArray[gameData.sequenceNum].answer[answerNum].dragEnable)){
				loopAudioArray.push({type:'answer', id:thisMode+'answerAudio'+answerNum, list:n});
				fileFest.push({src:loopTargetArray[gameData.sequenceNum].answer[answerNum].audio, id:thisMode+'answerAudio'+answerNum})
			}
		}
		
		for(var n=0;n<loopTargetArray[gameData.sequenceNum].input.length;n++){
			if(loopTargetArray[gameData.sequenceNum].input[n].type == 'image'){
				fileFest.push({src:loopTargetArray[gameData.sequenceNum].input[n].text, id:thisMode+'inputImage'+n, type: createjs.LoadQueue.IMAGE})
			}
		}
		
		if(loopTargetArray[gameData.sequenceNum].explanationType == 'image'){
			fileFest.push({src:loopTargetArray[gameData.sequenceNum].explanation, id:thisMode+'explanationImage', type: createjs.LoadQueue.IMAGE})
		}
		
		var explanationAudio = loopTargetArray[gameData.sequenceNum].explanationAudio;
		...
		if(explanationAudio != ''){
			...
			fileFest.push({src:loopTargetArray[gameData.sequenceNum].explanationAudio, id:thisMode+'explanationAudio'});
		}
	}
...
function getArrayValue(type, answerNum, n){
	...
	if(type == 'background'){
		...
		value.top = !checkValue(gameData.targetArray[gameData.sequenceNum].backgroundTop) ? 0 : gameData.targetArray[gameData.sequenceNum].backgroundTop;
		value.left = !checkValue(gameData.targetArray[gameData.sequenceNum].backgroundLeft) ? 0 : gameData.targetArray[gameData.sequenceNum].backgroundLeft;
		value.width = !checkValue(gameData.targetArray[gameData.sequenceNum].backgroundWidth) ? 100 : gameData.targetArray[gameData.sequenceNum].backgroundWidth;
		value.height = !checkValue(gameData.targetArray[gameData.sequenceNum].backgroundHeight) ? 100 : gameData.targetArray[gameData.sequenceNum].backgroundHeight;
		...
		value.dragEnable = gameData.targetArray[gameData.sequenceNum].answer[answerNum].dragEnable == 'false' ? false : true;
		value.dropEnable = gameData.targetArray[gameData.sequenceNum].answer[answerNum].dropEnable == 'false' ? false : true;
		value.dropLabelType = gameData.targetArray[gameData.sequenceNum].answer[answerNum].dropLabelType;
		value.dropLabelText = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[answerNum].dropLabelText) ? '' : gameData.targetArray[gameData.sequenceNum].answer[answerNum].dropLabelText;	
		value.dropLabelTop = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelTop) ? 0 : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelTop;
		value.dropLabelLeft = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelLeft) ? 0 : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelLeft;
		value.dropLabelWidth = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelWidth) ? 0 : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelWidth;
		value.dropLabelHeight = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelHeight) ? 0 : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelHeight;
		value.dropLabelFontSize = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelFontSize) ? dropLabelFontSize : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelFontSize;
		value.dropLabelLineHeight = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelLineHeight) ? dropLabelLineHeight : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelLineHeight;
		value.dropLabelColor = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelColor) ? dropLabelColor : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelColor;
		value.dropLabelAlign = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelAlign) ? dropLabelAlign : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelAlign;
		value.dropLabelOffsetTop = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelOffsetTop) ? dropLabelOffsetTop : gameData.targetArray[gameData.sequenceNum].answer[n].dropLabelOffsetTop;
		
		value.dropLeft = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropLeft) ? questionLeft : gameData.targetArray[gameData.sequenceNum].answer[n].dropLeft;
		value.dropTop = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropTop) ? questionTop : gameData.targetArray[gameData.sequenceNum].answer[n].dropTop;
		value.dropWidth = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropWidth) ? answerWidth : gameData.targetArray[gameData.sequenceNum].answer[n].dropWidth;
		value.dropHeight = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropHeight) ? answerHeight : gameData.targetArray[gameData.sequenceNum].answer[n].dropHeight;
		
		value.dropOffLeft = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropOffLeft) ? 0 : gameData.targetArray[gameData.sequenceNum].answer[n].dropOffLeft;
		value.dropOffTop = !checkValue(gameData.targetArray[gameData.sequenceNum].answer[n].dropOffTop) ? 0 : gameData.targetArray[gameData.sequenceNum].answer[n].dropOffTop;
		...
		value.dropMax = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].dropMax) ? groupDropMax : gameData.targetArray[gameData.sequenceNum].groups[n].dropMax;
		...
		value.dropWidth = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].dropWidth) ? groupDropWidth : gameData.targetArray[gameData.sequenceNum].groups[n].dropWidth;
		value.dropHeight = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].dropHeight) ? groupDropHeight : gameData.targetArray[gameData.sequenceNum].groups[n].dropHeight;
		
		value.dropOffLeft = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].dropOffLeft) ? groupDropOffLeft : gameData.targetArray[gameData.sequenceNum].groups[n].dropOffLeft;
		value.dropOffTop = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].dropOffTop) ? groupDropOffTop : gameData.targetArray[gameData.sequenceNum].groups[n].dropOffTop;
		...
		value.fontSize = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].fontSize) ? groupFontSize : gameData.targetArray[gameData.sequenceNum].groups[n].fontSize;
		value.lineHeight = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].lineHeight) ? groupLineHeight : gameData.targetArray[gameData.sequenceNum].groups[n].lineHeight;
		value.color = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].color) ? groupColor : gameData.targetArray[gameData.sequenceNum].groups[n].color;
		value.align = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].align) ? groupAlign : gameData.targetArray[gameData.sequenceNum].groups[n].align;
		value.offsetTop = !checkValue(gameData.targetArray[gameData.sequenceNum].groups[n].offsetTop) ? groupOffsetTop : gameData.targetArray[gameData.sequenceNum].groups[n].offsetTop;
	}
...
function buildGroups(){
	...
	for(n=0;n<gameData.targetArray[gameData.sequenceNum].groups.length;n++){
		...
		var groupDropWrapperHTML = "<div id='groupDrop"+n+"' class='groupDrop' style='width:"+value.dropWidth+"%; height:"+value.dropHeight+"%; top:"+value.dropTop+"%; left:"+value.dropLeft+"%; border:"+groupBorder+" solid "+groupStroke+"; background:"+groupBackground+";' data-left='"+dropLeft+"%' data-top='"+dropTop+"%' data-offleft='"+value.dropOffLeft+"%' data-offtop='"+value.dropOffTop+"%' data-width='"+value.dropWidth+"%' data-height='"+value.dropHeight+"%' data-max='"+value.dropMax+"' data-answer='"+value.correctAnswer+"' data-id='"+n+"'></div>";
	...
	for(n=0;n<gameData.targetArray[gameData.sequenceNum].answer.length;n++){
		...
		if(value.type == 'image'){
			...
			if(answerButtonBgEnable){
				var backgroundShadowHTML = "<div class='shadow resizeBorder' data-border='"+answerButtonBgRoundNumber+"' style='border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -moz-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -webkit-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; background:"+answerButtonBgShadowColour+"; width:100%; height:100%; position:absolute; top:"+value.offsetTop+"%; left:0;'></div>";
				$('#answer'+n).append(backgroundShadowHTML);
				
				var backgroundHTML = "<div class='background resizeBorder' data-border='"+answerButtonBgRoundNumber+"' style='border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -moz-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -webkit-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; background:"+answerButtonBgColour+"; width:100%; height:85%; position:absolute; top:"+value.offsetTop+"%; left:0;'></div>";
		...
		if(playerData.answerType == 'drag' && value.submit != 'true'){
			if(gameData.targetArray[gameData.sequenceNum].groups.length > 0){
				...
				var answerDropWrapperHTML = "<div id='drop"+n+"' class='drop' style='width:"+value.dropWidth+"%; height:"+value.dropHeight+"%; top:"+value.dropTop+"%; left:"+value.dropLeft+"%; border:"+dropBorder+" solid "+dropStroke+"; background:"+dropBackground+";' data-left='"+dropLeft+"%' data-top='"+dropTop+"%'></div>";
	...
	for(n=0;n<gameData.targetArray[gameData.sequenceNum].input.length;n++){
		...
		if(value.submit == 'true'){
			if(value.type == 'image'){
				...
				var inputWrapperHTML = "<div id='input"+n+"' class='input resizeFont resizeBorder' data-border='"+answerButtonBgRoundNumber+"' style='border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -moz-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -webkit-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; width:"+value.width+"%; height:"+value.height+"%; top:"+value.top+"%; left:"+value.left+"%;'></div>";
				...
				var clickHTML = "<div class='buttonClick resizeBorder' data-border='"+answerButtonBgRoundNumber+"' style='border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -moz-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; -webkit-border-radius: "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px "+answerButtonBgRoundNumber+"px; width:100%; height:100%; position:absolute; top:"+value.offsetTop+"%;'></div>";
...
function revertPosition(obj){
	if(obj.hasClass('occupied')){
		TweenMax.to(obj, dragRevertSpeed,{css:{left:obj.attr('data-left-drop'), top: obj.attr('data-top-drop')}});
	}else{
		TweenMax.to(obj, dragRevertSpeed,{css:{left:obj.attr('data-left'), top: obj.attr('data-top')}});
	}
}
								...
								$('#groupHolder .groupDrop').each(function(dropIndex, dropElement) {
									...
									if(answerInCorrectGroup != -1 && !foundInGroup){
										...
										if(answerInGroup == -1){
											...
											targetAnswer.find('.background').css('background', wrongButtonBgColour);
											targetAnswer.find('.shadow').css('background', wrongButtonBgShadowColour);
...
function toggleResult(con){
	if(con){
		$('.itemWinnerCup img').attr('src','assets/images/game_over.png');	
	}else{
		$('.itemWinnerCup img').attr('src','assets/images/game_over.png');
	}
}
...
function share(action){
	...
	if( action == 'twitter' ) {
		...
		shareurl = 'https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent(loc+'share.php?desc='+text+'&title='+title+'&url='+loc+'&thumb='+loc+'share4byod-dragNdrop-vake.png&width=590&height=300');
